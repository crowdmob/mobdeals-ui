...
MobDeals.Account = {
  ...
  assertCookied: function(callback, returnUrl) {
    this.cookied(function(isCookied) {
      ...
      else { MobDeals.Account.loginPrompt(callback, null, returnUrl); }
    });
  },
  
  cookied: function(callback) {
    if (this._cookied == false) { this._verifyCookie(callback); }
  ...
  _verifyCookie: function(callback) {
    ...
    $.ajax({
      ...
      xhrFields: {withCredentials: true},
      crossDomain: true,
      success: function(data) { MobDeals.Account._authenticated(data); },
      error: function(data) { MobDeals.Account._authenticated(null); },
